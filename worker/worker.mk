...
deploy-worker: ready-to-deploy worker-image push-worker-image delete-worker-deployment
	kubectl apply -f --overwrite=true ./worker/k8s/buildbot-secret.yaml
	kubectl apply -f --overwrite=true ./worker/k8s/buildkite-secret.yaml
	export WORKER_IMAGE=$(WORKER_IMAGE) \
	&& export BUILDBOT_MASTER="$(shell kubectl get route master-route-www -o json | jq -j '"http://"+.spec.host+.spec.path')" \
	...
	kubectl apply -f --overwrite=true ./out/worker-pod.yaml